...
function getWeather(){
  var city = cityName;
  var units ="m"
   fetch("http://api.weatherstack.com/current?access_key=518949f185a2bf09763c30b252dbc50d&query="+city+"&units="+units)
   .then(a => a.json())
  .then(response => {
...
function getData2(response){
  ...
  fetch("https://api.openweathermap.org/data/2.5/onecall?lat="+response.location.lat+"&lon="+response.location.lon+"&exclude=hourly,minutely&units=metric&appid=f3108c9ee1872ae74534614c51a2b117")
...
//   }
  ...
  if(response.current.is_day==='yes'){
    ...
    <marquee behavior="scroll" direction="left" style="color:rgb(15, 15, 15);font-family: cursive; font-size:20px; font-weight: bold; padding:5px ;">WELCOME To Weather Forcasting Page created By Diwakar Kharb, Anuj Sharma </marquee>
    `;
     imageEl.innerHTML=`
     body{
    ...
    document.getElementById("marque").innerHTML=`
    <marquee behavior="scroll" direction="left" style="color:rgb(248, 246, 246);font-family: cursive; font-size:20px; font-weight: bold; padding:5px ;">WELCOME To Weather Forcasting Page created By Diwakar Kharb, Anuj Sharma  </marquee>
    `;
    imageEl.innerHTML=`
    body{
